...
func (l Handlers) AnalyzeStateHandler(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	analyzerID, ok := params["analyzerId"]
	if !ok {
		l.writeError(w, fmt.Errorf("analyzerId is required"))
		return
	}

	k, err := l.logic.GraphAnalysis(r.Context(), shared.GraphAnalysisParams{
		Symbol: analyzerID,
	})
	if err != nil {
		l.writeError(w, err)
...
func klineDataZoomBoth() *charts.Kline {
	...
	kline.SetXAxis(x).AddSeries("kline", y).SetSeriesOptions(charts.WithMarkPointNameTypeItemOpts(opts.MarkPointNameTypeItem{
		Name:     "highest value",
		Type:     "max",
		ValueDim: "highest",
	}),
...
func (KlineExamples) Examples(w io.Writer) {
